...
import '../../controllers/sdk_configurations/sdk_configuration_general_settings_controller.dart';

class SDKConfigurationGeneralSettingsScreen extends StatefulWidget {
  const SDKConfigurationGeneralSettingsScreen({
    super.key,
    required this.controller,
  });

  final SDKConfigurationGeneralSettingsController controller;

  @override
  State<SDKConfigurationGeneralSettingsScreen> createState() =>
      _SDKConfigurationGeneralSettingsScreenState();
}

class _SDKConfigurationGeneralSettingsScreenState
    extends State<SDKConfigurationGeneralSettingsScreen> {
  ...
  Future<void> _loadSDKConfiguration() async {
    ...
    try {
      ...
      setState(() {
        ...
        _selectedColor = _resolveColor(sdkConfiguration.primaryColor);
      });
  ...
  Future<void> _saveGeneralSettings() async {
    ...
    try {
      final updated = await widget.controller.updateSDKConfigurationGeneralSettings(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Configuracion general de la Configuracion SDK actualizada correctamente.',
          ),
        ),
      );
    } on ArgumentError catch (error) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'No fue posible guardar la configuracion general de la Configuracion SDK.',
          ),
        ),
      );
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Configuracion General del SDK'),
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Visualiza y administra la configuracion basica de la Configuracion SDK.',
              ),
              ...
              if (_sdkConfiguration != null)
                Form(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _ReadOnlyField(label: 'Nombre actual', value: _sdkConfiguration!.name),
                      TextFormField(
