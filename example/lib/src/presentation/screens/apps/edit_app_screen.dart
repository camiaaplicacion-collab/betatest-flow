...
import '../../controllers/apps/edit_app_controller.dart';
...
class EditAppScreen extends StatefulWidget {
  ...
  final EditAppController controller;
...
class _EditAppScreenState extends State<EditAppScreen> {
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      final updated = await widget.controller.updateApp(
        appId: app.appId,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('App actualizada correctamente.')),
      );
    } on ArgumentError catch (error) {
      if (!mounted) {
        return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No fue posible actualizar la App.')),
      );
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Editar App')),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Carga una App existente y actualiza nombre y descripcion.'),
