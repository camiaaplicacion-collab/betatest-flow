...
import '../../controllers/apps/app_general_settings_controller.dart';

class AppGeneralSettingsScreen extends StatefulWidget {
  const AppGeneralSettingsScreen({
    super.key,
    required this.controller,
  });

  final AppGeneralSettingsController controller;

  @override
  State<AppGeneralSettingsScreen> createState() => _AppGeneralSettingsScreenState();
}

class _AppGeneralSettingsScreenState extends State<AppGeneralSettingsScreen> {
  ...
  Future<void> _loadApp() async {
    ...
    try {
      ...
      setState(() {
        ...
        _selectedColor = _resolveColor(app.primaryColor);
      });
  ...
  Future<void> _saveGeneralSettings() async {
    final app = _app;
    if (app == null || _isLoading || _isSaving) {
      return;
    }
    ...
    try {
      final updated = await widget.controller.updateAppGeneralSettings(
        appId: app.appId,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Configuracion general de la App actualizada correctamente.'),
        ),
      );
    } on ArgumentError catch (error) {
      if (!mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No fue posible guardar la configuracion general de la App.'),
        ),
      );
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Configuracion General de la App'),
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Visualiza y administra la configuracion basica de la App.',
              ),
              ...
              Form(
                ...
                child: Row(
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: _isLoading || _isSaving ? null : _loadApp,
                      child: _isLoading
                          ? const SizedBox(
                              width: 16,
                              height: 16,
              ...
              if (_app != null)
                Form(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _ReadOnlyField(label: 'Nombre actual', value: _app!.name),
                      TextFormField(
                      ...
                      const SizedBox(height: 12),
                      _ReadOnlyField(label: 'appId', value: _app!.appId),
                      _ReadOnlyField(label: 'workspaceId', value: _app!.workspaceId),
                      _ReadOnlyField(label: 'status', value: _app!.status),
                      _ReadOnlyField(
                        label: 'createdAt',
                        value: _app!.createdAt.toIso8601String(),
                      ),
                      _ReadOnlyField(
                        label: 'updatedAt',
                        value: _app!.updatedAt.toIso8601String(),
                      ),
                      const SizedBox(height: 12),
                      SizedBox(
